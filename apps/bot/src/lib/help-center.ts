...
export const COMMAND_DOCS: HelpCommandDoc[] = [
  { name: 'ping', description: 'Check gateway and response latency.', usage: '/ping [hide:true|false] | !ping', category: 'Core' },
  { name: 'info', description: 'Platform overview, links and quick actions.', usage: '/info | !info', category: 'Core' },
  { name: 'help', description: 'Browse command help by category or command.', usage: '/help [category] [command] | !help [category|command]', category: 'Core' },
  { name: 'avatar', description: 'Show a user avatar in high quality.', usage: '/avatar [user] | !avatar [@user]', category: 'Utility' },
  { name: 'server', description: 'Show current server snapshot.', usage: '/server | !server', category: 'Utility' },
  { name: 'boop', description: 'Playful social interaction command.', usage: '/boop user:@someone | !boop @user', category: 'Social' },
];
...
export function buildHelpOverview(page: number, prefix: string) {
  ...
  const actions = new ActionRow<Button>().addComponents(
    new Button().setCustomId(`help:page:${Math.max(0, safePage - 1)}`).setStyle(ButtonStyle.Secondary).setLabel('◀ Prev').setDisabled(safePage === 0),
    new Button().setCustomId(`help:page:${Math.min(max - 1, safePage + 1)}`).setStyle(ButtonStyle.Secondary).setLabel('Next ▶').setDisabled(safePage === max - 1)
  );
